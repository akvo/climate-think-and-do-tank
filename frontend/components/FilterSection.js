...
import { CalenderIcon, RegionIcon, TopicIcon, TypeIcon } from './Icons';
...
export default function FilterSection({
  ...
  return (
    <div
      className={`bg-white rounded-full p-1 shadow-lg backdrop-blur-sm ${className}`}
      ref={dropdownRef}
    >
      <div className="flex items-center">
        {visibleFilters.map((filterKey, index) => {
          ...
          const isAllSelectedInTemp =
            config.hasAllOption &&
            allValues.every((value) => tempSelections_current.includes(value));
          return (
            <React.Fragment key={filterKey}>
              ...
              {index < visibleFilters.length - 1 && (
                <div className="h-8 w-px bg-gray-200 mx-2 flex-shrink-0"></div>
              )}
            </React.Fragment>
          );
        })}
        ...
        {hasActiveFilters && (
          <>
            ...
            >
              <RotateCcw className="w-4 h-4 group-hover:rotate-180 transition-trans4orm duration-300" />
